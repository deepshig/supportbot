...
def neural_network(question):
    ...
    out = frappe.db.sql("""SELECT name, fitness, MATCH(raw) AGAINST(%s IN NATURAL LANGUAGE MODE) AS score FROM tabposts WHERE MATCH(raw) AGAINST(%s IN NATURAL LANGUAGE MODE) ORDER BY score DESC, fitness DESC LIMIT %s ;""", (question, question, max_answers), as_dict=True, debug=True)
...
def answer(question, i=0):
    ...
    # syn0 = 2*np.random.randn((7,max_answers)) - 1
    # syn1 = 2*np.random.randn((max_answers,1)) - 1
